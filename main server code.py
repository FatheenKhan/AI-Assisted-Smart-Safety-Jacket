...
classifier = joblib.load('PATH OF THE PICKLE FILE OF THE HEART RATE CLASSIFIER')
...
stored_audio = None
cred = credentials.Certificate(r"specify the location of the json file")
firebase_admin.initialize_app(cred, {'YOUR DATABASE URL'})
#fetch data (i.e firebase implementation)

def fetch_mq2_sensor_reading():
    ref_path = 'PATH OF THE ATTRIBUTES'
...
def mylocation():
    api_key = 'API KEY FROM THE GOOGLE MAPS'
...
def weather(latitude,longitude):
    ...
    API_KEY = "API KEY FROM THE OPEN WEATHER APP"
    LATITUDE = latitude
    LONGITUDE = longitude
